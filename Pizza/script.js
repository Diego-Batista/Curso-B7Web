...
pizzaJson.map((item, index) => {
    let pizzasItem = c('.models .pizza-item').cloneNode(true);

    // PREENCHER AS INFORMAÇÕES EM PIZZAITEM
    pizzasItem.setAttribute('data-key', index);
    pizzasItem.querySelector('.pizza-item--img img').src = item.img;
    pizzasItem.querySelector('.pizza-item--price').innerHTML = `R$ ${item.price[2].toFixed(2)}`;
    ...
    c('.pizza-area').append(pizzasItem);
...
cs('.pizzaInfo--size').forEach((size, sizeIndex) => {
    size.addEventListener('click', () => {
		...
		c(".pizzaInfo--actualPrice").innerHTML = `R$ ${pizzaJson[modalKey].price[
			sizeKey
		].toFixed(2)}`;
    });
});
